...
def test_duckdb_retains_versions_and_loads_latest(tmp_path) -> None:
    ...
    storage.save_market_data(
        _frame(101.0), source="yfinance", interval="1d", available_timestamp=first
    )
    storage.save_market_data(
        _frame(102.0), source="yfinance", interval="1d", update=True, available_timestamp=second
    )
    latest = storage.load_market_data(source="yfinance", interval="1d", symbol="NVDA")
    history = storage.load_market_data(
        source="yfinance", interval="1d", symbol="NVDA", history=True
    )
...
def test_duckdb_as_of_prevents_future_knowledge(tmp_path) -> None:
    ...
    known = storage.load_market_data(
        source="yfinance", interval="1d", symbol="NVDA", as_of=datetime(2026, 1, 4, tzinfo=UTC)
    )

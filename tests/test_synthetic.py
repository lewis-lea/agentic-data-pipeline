...
def test_cash_reinvestment_and_correlated_shock():
    ...
    for frame in histories.values():
        ...
        np.testing.assert_allclose(
            (price + cash).iloc[1:] / price.iloc[:-1].to_numpy(),
            adjusted.iloc[1:] / adjusted.iloc[:-1].to_numpy(),
            rtol=1e-12,
        )
        ...
        assert price.loc["2020-03-23"] / price.loc["2020-02-19"] < 0.85
    ...
    assert returns.corr().to_numpy()[np.triu_indices(6, 1)].mean() > 0.05
...
def test_snapshot_offline_reproducibility_and_export_provenance(tmp_path, monkeypatch):
    ...
    stored = ParquetStorage(tmp_path / "datasets").load_dataset(
        source="synthetic", dataset="corporate_actions", symbol="SIM-A"
    )

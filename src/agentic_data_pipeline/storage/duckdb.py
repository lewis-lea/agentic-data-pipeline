...
class DuckDBStorage:
    ...
    def save_market_data(
        ...
        *,
        ...
        layer: str = "raw",
        update: bool = False,
        available_timestamp: datetime | pd.Timestamp | None = None,
    ) -> Path:
        ...
        with self._connect() as connection:
            ...
            connection.execute(
                "INSERT OR IGNORE INTO market_data SELECT * FROM _incoming_market_data"
            )
    ...
    def load_market_data(
        ...
        *,
        ...
        if result.empty:
            result = pd.DataFrame(
                columns=["open", "high", "low", "close", "volume", "source", "available_timestamp"],
    ...
    def latest_event_timestamp(
        self, *, source: str, interval: str, symbol: str
    ) -> pd.Timestamp | None:
        """Return the newest persisted event timestamp."""
        with self._connect() as connection:
            row = connection.execute(
                "SELECT max(event_timestamp) FROM market_data WHERE symbol = ? AND interval = ? AND source = ?",
                [symbol.strip().upper(), interval, source],
            ).fetchone()

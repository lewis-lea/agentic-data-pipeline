...
def create_corporate_actions(
    ...
    *,
    ...
    for source, target in ACTION_COLUMNS.items():
        result[target] = (
            pd.to_numeric(history[source], errors="raise") if source in history else np.nan
        )
    ...
    result.index = pd.DatetimeIndex(dates, tz="UTC", name="date")
    ...
    result.attrs = {
        ...
        "available_fields": [
            target for source, target in ACTION_COLUMNS.items() if source in history
        ],
    }
    return result

...
PROFILES = {
    "GRG.L": (10.0, [1, 1.05, 1.25, 1.25, 2.2, 2.0, 2.9, 2.4, 2.9, 3.2, 2.2], .025, .013),
    "DNLM.L": (8.0, [1, .95, .85, .75, 1.2, 1.45, 1.65, 1.3, 1.65, 1.75, 1.65], .04, .012),
    "SCT.L": (3.0, [1, 1.1, 1.5, 2.1, 3.2, 4.8, 6.3, 4.9, 5.6, 6.6, 7.2], .02, .014),
    "CURY.L": (4.0, [1, .85, .6, .38, .4, .35, .4, .23, .2, .3, .4], .015, .018),
    "MNDI.L": (13.0, [1, 1.2, 1.4, 1.45, 1.4, 1.5, 1.55, 1.3, 1.4, 1.2, .95], .035, .012),
    "RMV.L": (4.0, [1, 1.1, 1.2, 1.1, 1.6, 1.65, 1.9, 1.6, 1.75, 1.9, 1.85], .015, .011),
}
...
def _noise(rng: np.random.Generator, size: int, sigma: float) -> np.ndarray:
    ...
    for index in range(1, size):
        values[index] = .96 * values[index - 1] + rng.normal(0, sigma)
...
def generate_histories(seed: int = SEED) -> dict[str, pd.DataFrame]:
    ...
    common = _noise(np.random.default_rng(seed), len(dates), .006)
    # Authored abrupt downturn/recovery and a later inflation-like drawdown.
    shock_dates = pd.to_datetime([START, "2020-02-19", "2020-03-23", "2021-03-01",
                                  "2022-01-03", "2022-10-03", "2023-07-03", END])
    shocks = np.interp(dates.asi8, shock_dates.asi8, [0, 0, -.42, 0, 0, -.18, 0, 0])
    result = {}
    for symbol, (initial, levels, cash_yield, sigma) in PROFILES.items():
        ...
        latent = initial * np.exp(np.interp(x, positions, np.log(levels))
                                 + common + _noise(rng, len(dates), sigma) + shocks)
        ...
        for index, day in enumerate(dates):
            ...
            if (day.month in (5, 11) and day.year != 2020
        ...
        result[symbol] = pd.DataFrame({"Close": prices, "Adj Close": adjusted,
            "Dividends": cash, "Capital Gains": 0.0, "Stock Splits": 0.0}, index=dates)
    return result
...
def build_synthetic_snapshot(catalogue: dict[str, Any], *, seed: int = SEED) -> dict[str, Any]:
    ...
    snapshot = build_snapshot(catalogue, loader=lambda symbol: (histories[symbol], {"currency": "GBP"}))
    snapshot.update(data_source="synthetic", synthetic=True,
        ...
        simulation={"version": GENERATOR_VERSION, "seed": seed, "start": START, "end": END,
                    ...
                    "profiles": {symbol: {"initial_gbp": profile[0], "trend_anchors": profile[1],
                        "annual_cash_yield": profile[2], "daily_noise": profile[3]}
